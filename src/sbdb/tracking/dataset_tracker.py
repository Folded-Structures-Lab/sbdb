...
def get_file_metadata(csv_path, json_path=None):
    ...
    if csv_path and Path(csv_path).exists():
        ...
        try:
            metadata["csv_size_mb"] = round(
                Path(csv_path).stat().st_size / (1024 * 1024), 2
            )
        except Exception:
    ...
    if json_path and Path(json_path).exists():
        ...
        try:
            metadata["json_size_mb"] = round(
                Path(json_path).stat().st_size / (1024 * 1024), 2
            )
        except Exception:
...
class DatasetTracker:
    ...
    def __init__(
        ...
        csv_subdir="collections_csv",
        json_subdir="collections_json",
        ...
        Args:
            base_dir: Base directory for repository (defaults to parent of current file)
    ...
    def get_package_versions(self):
        ...
        """
        return {"main_package": "Unknown", "framework_package": "Unknown"}
    ...
    def update_generation_record(self, collection_name, operation_type, notes=None):
        ...
        if not mask.any():
            print(
                f"Warning: Collection {collection_name} not found in record. Adding new entry..."
            )
            # Add new entry
            category, dataset_type = self.categorise_collection(collection_name)
            new_record = {
        ...
        if operation_type == "generation":
            ...
            df.loc[mask, "main_package_version"] = package_versions.get(
                "main_package", "Unknown"
            )
            df.loc[mask, "framework_package_version"] = package_versions.get(
        ...
        if notes:
            current_notes = (
                df.loc[mask, "notes"].iloc[0]
                if not pd.isna(df.loc[mask, "notes"].iloc[0])
                else ""
            )
            df.loc[mask, "notes"] = (
                f"{current_notes}; {notes}" if current_notes else notes
            )
    ...
    def get_generation_status(self):
        ...
        if total_collections > 0:
            print(
                f"Generated: {generated} ({generated / total_collections * 100:.1f}%)"
            )
            print(
                f"Dataset Verified: {verified} ({verified / total_collections * 100:.1f}%)"
            )
            print(
                f"Database Populated: {populated} ({populated / total_collections * 100:.1f}%)"
            )
            print(
                f"Database Verified: {db_verified} ({db_verified / total_collections * 100:.1f}%)"
            )
