...
def main():
    ...
    try:
        ...
        print(
            f"   ✓ Design variable categories: {list(des_vars.design_var_sets.keys())}"
        )
        ...
        print(
            f"   ✗ Error: Could not find '{input_file}'. Please ensure the file exists."
        )
        return
    ...
    if hasattr(object_set, "skipped_indices") and object_set.skipped_indices:
        print(
            f"⚠Skipped {len(object_set.skipped_indices)} parameter combos due to errors"
        )
